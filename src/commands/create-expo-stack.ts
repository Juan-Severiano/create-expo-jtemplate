...
const command: GluegunCommand = {
  ...
  run: async (toolbox) => {
    ...
    const printSomethingWentWrong = () => {
      ...
      info(
        `\nIf this was unexpected, please open an issue: https://github.com/Juan-Severiano/create-expo-jtemplate#reporting-bugs--feedback`
      );
      info('');
    };
    ...
    try {
      ...
      if (!first && (options.ignite || !(useDefault || optionsPassedIn || skipCLI || useBlankTypescript))) {
        ...
        cliResults.projectName = pathSegments.pop();
      }
